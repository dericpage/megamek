...
import static megamek.common.logging.LogLevel.INFO;
...
public class GameOptions extends AbstractOptions {
    ...
    // Make this package-private to use with unit testing and pass in a FakeLogger. 
    private GameOptions(final MMLogger logWith) {
        super();
        if (null != logger) {
            logger = logWith;
        }
    ...
    public GameOptions copy() {
        final GameOptions copy = new GameOptions(logger);
        copy.initialize();

        LOCK.readLock().lock();
        try {
            final Enumeration<IBasicOptionGroup> groupEnumeration = getOptionsInfoImp().getGroups();
            while (groupEnumeration.hasMoreElements()) {
                final IBasicOptionGroup group = groupEnumeration.nextElement();
                final Enumeration<String> nameEnumeration = group.getOptionNames();
                while (nameEnumeration.hasMoreElements()) {
                    final String optionName = nameEnumeration.nextElement();
                    final IOption option = getOption(optionName);
                    copy.addOption(group, optionName, option.getType(), option.getDefault());
                }
            }
        } finally {
    ...
    public void initialize() {
        ...
        try {
            ...
            addOption(allowed,
                      OptionsConstants.ALLOWED_TECHLEVEL,
                      IOption.CHOICE,
                      TechConstants.T_SIMPLE_NAMES[TechConstants.T_SIMPLE_STANDARD]); //$NON-NLS-1$
            ...
            addOption(advancedGroundMovement,
                      OptionsConstants.ADVGRNDMOV_TACOPS_PHYSICAL_ATTACK_PSR,
                      false); //$NON-NLS-1$
            ...
            addOption(advancedGroundMovement,
                      OptionsConstants.ADVGRNDMOV_TACOPS_FAST_INFANTRY_MOVE,
                      false); //$NON-NLS-1$
            ...
            addOption(advancedGroundMovement,
                      OptionsConstants.ADVGRNDMOV_VEHICLE_ADVANCED_MANEUVERS,
                      false); //$NON-NLS-1$
    ...
    public Vector<IOption> loadOptions(final File file,
                                       final boolean print) {
        ...
        try {
            ...
            logger.log(getClass(), "loadOptions(File, boolean)", ERROR,
                       "Error loading XML for game options.", ex);
        }

        return changedOptions;
    }

    private IOption parseOptionNode(final IBasicOption node,
                                    final boolean print) {
            ...
            if (null != tempOption) {
                if (!tempOption.getValue().toString()
                    ...
                    try {
                        ...
                        if (print) {
                            logger.log(getClass(), methodName, INFO,
                                       "Set option '" + name + "' to '" + value + "'.");
                        }

                        option = tempOption;
                    } catch (final IllegalArgumentException iaEx) {
                        logger.log(getClass(), methodName, ERROR, "Error trying to load option '" + name +
                                                                  "' with a value of '" + value + "'.", iaEx);
                    }
                }
            } else {
                logger.log(getClass(), methodName, WARNING, "Invalid option '" + name +
    ...
    public static void saveOptions(final Vector<IBasicOption> options,
        ...
        try {
            ...
            logger.log(GameOptions.class, "saveOptions(Vector<IBasicOption>, String)", ERROR,
    ...
    private static class GameOptionsXML {
        ...
        public Vector<IBasicOption> getOptions() {
            return new Vector<>(Collections.unmodifiableCollection(options));
        }
    }
}
